        ...
        switch lower(nodeType)
            ...
            case {'data', 'rawdata', 'datalist', 'channel', 'headmodel', 'noisecov', 'ndatacov', 'results', 'kernel', 'matrix', 'matrixlist', 'dipoles', 'timefreq', 'spectrum', 'pdata', 'presults', 'ptimefreq', 'pspectrum', 'pmatrix', 'link', 'image', 'video', 'videolink'}
                ...
                if isRead
                    ...
                    isRead = ProtocolInfo.iReadFiles(iFile) == bstNodes(1).getLastModifiedTime();
                end
                % Mark node as "read"
                if ~isRead
                    ProtocolInfo.iReadFiles(iFile) = uint32(bstNodes(1).getLastModifiedTime());
            ...
            case 'channel'
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ~bst_get('ReadOnly') && ~isempty(DisplayMod) && ~any(ismember(DisplayMod, {'MEG', 'MEG MAG', 'MEG GRAD'}))
                        AddSeparator(jPopup);
                        if (sSubject.Id == 0) || sSubject.UseDefaultAnat
                ...
                if ~bst_get('ReadOnly')
                    ...
                    if ~isAtlas && (length(bstNodes) == 1)
                        ...
                        if ~bstNodes(1).isMarked() && ~isempty(sSubject.iAnatomy)
                            ...
                            gui_component('MenuItem', jMenuRegister, [], '<HTML><B>SPM12</B>: Register + reslice', IconLoader.ICON_ANATOMY, [], @(h,ev)MriCoregister(filenameRelative, [], 'SPM', 1));
                            gui_component('MenuItem', jMenuRegister, [], '<HTML><B>SPM12</B>: Register only',      IconLoader.ICON_ANATOMY, [], @(h,ev)MriCoregister(filenameRelative, [], 'SPM', 0));
                            if isCt
                                gui_component('MenuItem', jMenuRegister, [], '<HTML><B>CT2MRI</B>: Register + reslice', IconLoader.ICON_ANATOMY, [], @(h,ev)MriCoregister(filenameRelative, [], 'CT2MRI', 1));
                                gui_component('MenuItem', jMenuRegister, [], '<HTML><B>CT2MRI</B>: Register only',      IconLoader.ICON_ANATOMY, [], @(h,ev)MriCoregister(filenameRelative, [], 'CT2MRI', 0));
                            end
                ...
                if (length(bstNodes) == 1)
                    ...
                    if strcmpi(DataType, 'data')
                        ...
                        for iMod = 1:length(DisplayMod)
                            ...
                            if strcmpi(DisplayMod{iMod}, 'EEG') && ~isempty(sSubject) && ~isempty(sSubject.iScalp)
                                sAnatFile = db_get('AnatomyFile', sSubject.iScalp, 'FileName');
                                gui_component('MenuItem', jMenuModality, [], 'Display on scalp', IconLoader.ICON_SURFACE_SCALP, [], @(h,ev)view_surface_data(sAnatFile.FileName, filenameRelative, 'EEG'));
                            % SEEG/ECOG: Display on cortex or MRI
                            elseif ismember(DisplayMod{iMod}, {'SEEG', 'ECOG', 'ECOG+SEEG'}) && ~isempty(sSubject)
                                if ~isempty(sSubject.iCortex)
                                    sAnatFile = db_get('AnatomyFile', sSubject.iCortex, 'FileName');
...
%% ===== MENU: PROJECT ON DEFAULT ANATOMY =====
    ...
    function fcnPopupProjectSources(isSeparator)
        ...
        % If more than one subject: just check if the subjects are using default anatomy
            ...
            sSubjects = db_get('Subject', SubjectIds, 'UseDefaultAnat');
...
%% ===== MENU: ALIGN SURFACES MANUALLY =====
    function fcnPopupAlign()
        ...
        for ix = 1:length(sAnatsAlign)
            if isempty(strfind(sAnatsAlign(ix).FileName, '_volatlas')) && isempty(strfind(sAnatsAlign(ix).FileName, '_volct'))
...
function fcnMriSegment(jPopup, sSubject, iSubject, iAnatomy, isAtlas, isCt)
    import org.brainstorm.icon.*;
    sAnatFiles = db_get('AnatomyFilesWithSubject', iSubject, 'Id', 'volume', 'Image');
    ...
    if ~isAtlas
        ...
        if (length(iAnatomy) <= 1) && isCt
            if ~isempty(sSubject.iAnatomy)
                sAnatFile = db_get('AnatomyFile', sSubject.iAnatomy, 'FileName');
                gui_component('MenuItem', jMenu, [], '<HTML><B>SPM12</B>: Skull stripping', IconLoader.(volIcon), [], @(h,ev)MriSkullStrip(MriFile, sAnatFile.FileName, 'SPM'));
                gui_component('MenuItem', jMenu, [], '<HTML><B>BrainSuite</B>: Skull stripping', IconLoader.(volIcon), [], @(h,ev)MriSkullStrip(MriFile, sAnatFile.FileName, 'BrainSuite'));
            end
    ...
    % === TISSUE SEGMENTATION ===
    elseif (length(iAnatomy) == 1) && ~isempty(strfind(lower(sAnatFile.Comment), 'tissues'))
...
function fcnPetProcessing(jPopup, sSubject, iAnatomy)
    ...
    if length(iAnatomy) == 1
        sAnatFile = db_get('AnatomyFile', iAnatomy, {'FileName', 'Comment'});
        gui_component('MenuItem', jMenu, [], 'Realign frames', IconLoader.ICON_VOLPET, [], @(h,ev)PetImportProcess_Callback(sAnatFile.FileName));
    end
end
